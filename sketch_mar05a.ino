...
<body>
  <div class="form-style-5 slideInDown animated">
    ...
    <form onsubmit="sendData(); return false;">
      <fieldset>
        ...
        <button type="button" name="fingerid_add" class="fingerid_add" onclick="enrollFingerprint()">Add Fingerprint
          ID</button>
      </fieldset>
      <fieldset>
   ...
   <form onsubmit="deleteFingerprint(); return false;">
    ID fingerprint: <input type="text" name="txtInput" id="iddelete" value="" />
  ...
  function sendData() {
    ...
    var url = baseUrl + 'senddata?fingerid=' + fingerid + '&name=' + name + '&number=' + number + '&email=' + email;
...
void handleCheckAttendance() {
  ...
  if (action.equals("attendance")) {
    Serial.println("You are checked");
  ...
  if (action.equals("endattendance")) {
    Serial.println("You are unchecked");
  }
}
void handleSendData() {
  String fingerid = server.arg("fingerid");
